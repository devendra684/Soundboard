...
import { Users, Mic, Music, Volume2, VolumeX, Trash2, ArrowUp, ArrowDown, UserPlus } from "lucide-react";
...
export default function RoomSessionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [mixOrder, setMixOrder] = useState<string[]>([]);
  ...
  // Update selection and order when loops change
  useEffect(() => {
    if (!loops) return;
    // Always reset selectedTrackIds and mixOrder to match the latest loops
    const sorted = [...loops].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
    setSelectedTrackIds(sorted.map((l: any) => l.id));
    setMixOrder(sorted.map((l: any) => l.id));
  }, [loops]);

  // Handlers for selection and ordering
  const toggleTrackSelection = (id: string) => {
    setSelectedTrackIds((prev) => prev.includes(id) ? prev.filter(tid => tid !== id) : [...prev, id]);
    setMixOrder((prev) => prev.includes(id) ? prev.filter(tid => tid !== id) : [...prev, id]);
  };
  ...
  const openMixPanel = () => {
    if (!loops) return;
    setPanelSelectedIds(loops.map((l: any) => l.id));
    const orderObj: { [id: string]: number } = {};
    const volumeObj: { [id: string]: number } = {};
    loops.forEach((l: any, idx: number) => {
      orderObj[l.id] = idx + 1;
  ...
  const handlePanelCheckbox = (id: string) => {
    setPanelSelectedIds((prev) => prev.includes(id) ? prev.filter(tid => tid !== id) : [...prev, id]);
  };
  ...
  const handlePanelConfirm = async () => {
    ...
    const selected = panelSelectedIds
      ...
      .map(({ id }) => loops.find((l: any) => l.id === id))
      .filter(Boolean);
    ...
    try {
      ...
      const buffers = await Promise.all(
        sortedSelected.map((l: any) => fetch(l.url).then((r: any) => r.arrayBuffer()))
      );
      ...
      <div className="flex items-center justify-between px-8 py-4 border-b border-white/10">
        ...
        <div className="flex items-center gap-2 text-white/80">
          ...
          <InviteModal code={room.code}>
            <button type="button" className="ml-2 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors" title="Invite to Room">
            ...
            <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
              <div className="relative bg-[#23124d] rounded-2xl p-8 shadow-xl border border-white/10 w-full max-w-2xl mx-auto mt-16">
                ...
                <div className="space-y-4">
                  {loops && loops.map((track: any, idx: number) => (
                          ...
                          >
                            {Array.from({ length: panelSelectedIds.length }, (_, i) => i + 1).map((n) => (
                              <option
                                key={n}
                                value={n}
                                // No longer disable, allow duplicates for error handling
                              >
                                {n}
                              </option>
                            ))}
